...
class Critic(nn.Module):
    ...
    def forward_with_feature(self, pixel_feats, query_feats):
        net = torch.cat([pixel_feats, query_feats], dim=-1)
        feat = F.leaky_relu(self.mlp1(net))
        net = self.mlp2(feat).squeeze(-1)
        return net, feat
...
class Network(nn.Module):
    def __init__(self, feat_dim):
        ...
        self.confidence = Confidence(feat_dim)
    ...
    def forward(self, pcs, dirs1, dirs2):
        ...
        conf_result_logits = self.confidence(net, input_queries)

        return pred_result_logits, conf_result_logits, whole_feats
    ...
    def inference_whole_pc(self, feats, dirs1, dirs2):
        ...
        conf_result_logits = self.confidence(feats, input_queries)

        soft_pred_results = torch.sigmoid(pred_result_logits)
        soft_pred_results = soft_pred_results.reshape(batch_size, num_pts)

        soft_conf_results = torch.sigmoid(conf_result_logits)
        soft_conf_results = soft_conf_results.reshape(batch_size, num_pts)

        return soft_pred_results, soft_conf_results

    def inference_whole_pc_confidence(self, feats, dirs1, dirs2):
        num_pts = feats.shape[-1]
        batch_size = feats.shape[0]
    ...
    def inference(self, pcs, dirs1, dirs2):
        ...
        conf_results = torch.sigmoid(conf_result_logits)

        return pred_results, conf_results
