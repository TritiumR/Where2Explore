...
parser.add_argument('--divide_state', action="store_true", default=False)
        ...
        if cat in conf.category_types:
            for primact_type in conf.primact_types:
                for epoch in range(conf.starting_epoch, conf.starting_epoch+conf.num_epochs):
                    if conf.divide_state:
                        state = str(epoch % 6)
                    for cnt_id in range(cat2freq[cat]):
                        datagen.add_one_collect_job(conf.data_dir, shape_id, cat, cnt_id, primact_type, epoch, state=state)
