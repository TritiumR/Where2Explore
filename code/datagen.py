        ...
        for todo in todos:
            if todo[0] == 'COLLECT':
                if todo[8] is not None:
                    cmd = 'xvfb-run -a python collect_data.py %s %s %d %s --out_dir %s --trial_id %d --random_seed %d --no_gui --state %s > /dev/null 2>&1' \
                          % (todo[1], todo[2], todo[3], todo[4], todo[5], todo[6], todo[7], todo[8])
                else:
                    cmd = 'xvfb-run -a python collect_data.py %s %s %d %s --out_dir %s --trial_id %d --random_seed %d --no_gui > /dev/null 2>&1' \
